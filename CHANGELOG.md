...
### Added
- Full public surface for v0.1.0 — `Vatverify` class with `validate`, `validateBatch`, `decide`, `rates.list`, `rates.get`, `health` methods.
...
- `NPM_TOKEN` secret configured on the repo
- Manual review of the full implementation
- First `npm publish` via the tag-triggered workflow

[Unreleased]: https://github.com/vatverify/node/compare/HEAD...HEAD
